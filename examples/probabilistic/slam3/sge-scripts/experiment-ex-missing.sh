...
if [ $# -ne 8 ]; then
  echo "usage: experiment-ex-missing [ore-slam-peaked|ore-slam-non-peaked|aisle-slam] [mm|opt] <ncols> <nrows> <nparticles> <po> <kappa> <tracker>"
  exit 1
fi
...
$root/slam --${slam_type} --seed $RANDOM -c $nc -r $nr --pa .9 --po $po --kappa $kappa --tracker="${type}-rbpf(nparticles=$np,force-resampling,sus=false,inference=$tracker)" -t 1
